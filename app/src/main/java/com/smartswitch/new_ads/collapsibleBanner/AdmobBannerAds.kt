...
class AdmobBannerAds {
    ...
    fun loadBannerAds(activity: Activity?, adsPlaceHolder: FrameLayout, admobAdaptiveIds: String, adEnable: Int, collapsiblePositionType: CollapsiblePositionType = CollapsiblePositionType.NONE, bannerCallBack: BannerCallBack) {
        activity?.let { mActivity ->
            try {
                if ( admobAdaptiveIds.isNotEmpty()) {
    ...
    private fun displayBannerAd(adsPlaceHolder: FrameLayout) {
        try {
            if (adaptiveAdView != null && adaptiveAdView!!.parent == null) {
                val viewGroup: ViewGroup? = adaptiveAdView?.parent as ViewGroup?
                viewGroup?.removeView(adaptiveAdView)
