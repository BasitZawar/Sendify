...
import com.smartswitch.databinding.FragmentMediaBinding
...
class MediaFragment : Fragment(), OnMediaItemClickCallback {
    ...
    private fun updateSendBtnUI() {
        if (getSelectedMediaListSize() > 0 || getSelectedContactsListSize() > 0) {
            ...
            binding.containerDataSize.visible()
            binding.containerSelectedFiles.visible()
            //binding.selectedTextView.text = "${getSelectedMediaListSize() + getSelectedContactsListSize()} ${getString(R.string.files)} / ${getTotalSize()}"
            binding.tvDataSize.text = "Data Size : ${getTotalSize()}"
            binding.tvSelectedFiles.text =
                "Selected Files : ${getSelectedMediaListSize() + getSelectedContactsListSize()} ${
                    getString(R.string.files)
                }"
        } else {
            binding.sendButton.disable()
            binding.sendButton.gone()
            binding.containerDataSize.gone()
            binding.containerSelectedFiles.gone()
            //binding.selectedTextView.text = "${getSelectedMediaListSize() + getSelectedContactsListSize()} ${getString(R.string.files)} / ${getTotalSize()}"
            binding.tvDataSize.text = "Data Size : ${getTotalSize()}"
            binding.tvSelectedFiles.text =
                "Selected Files : ${getSelectedMediaListSize() + getSelectedContactsListSize()} ${
